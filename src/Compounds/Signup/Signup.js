...
const Signup = () => {
  ...
  const [thumbnailOrder, setThumbnailOrder] = useState(images); // default order
 ...
 const handleUpClick = () => {
  ...
  const newIdx = idx > 0 ? idx - 1 : thumbnailOrder.length - 1;
  setMainImage(thumbnailOrder[newIdx]);
};

const handleDownClick = () => {
  const idx = thumbnailOrder.findIndex((img) => img === mainImage);
  const newIdx = idx < thumbnailOrder.length - 1 ? idx + 1 : 0;
  setMainImage(thumbnailOrder[newIdx]);
};
